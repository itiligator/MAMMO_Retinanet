...
def create_callbacks(model, training_model, prediction_model, validation_generator, args):
    ...
    save_path = args.annotations.split('/')[-2] + '_' + args.annotations.split('/')[-1].split('.')[0] + '_0' + '.h5'
    if os.path.isfile(os.path.join(args.snapshot_path,save_path)):
        cut = save_path.split('_')
        now_epoch = int(cut[-1].split('.')[0]) + 1
        save_path = args.annotations.split('/')[-2] + '_' + args.annotations.split('/')[-1].split('.')[0] + '_' + str(now_epoch) + '.h5'
...
def parse_args(args):
    ...
    csv_parser.add_argument('datatype', help='Datatype.')
